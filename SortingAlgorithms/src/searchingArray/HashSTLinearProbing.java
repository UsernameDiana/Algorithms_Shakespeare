...
public class HashSTLinearProbing<Key, Value> {
    ...
    public HashSTLinearProbing() {
        keys = (Key[]) new Object[M];
        vals = (Value[]) new Object[M];
    } 

    private HashSTLinearProbing(int cap) {
...
//  together in the way just described for String values.
    ...
    private void resize(int cap) {
        HashSTLinearProbing<Key, Value> t;
        t = new HashSTLinearProbing<Key, Value>(cap);
        for (int i = 0; i < M; i++) {
            if (keys[i] != null) {
                t.put(keys[i], vals[i]);
            }
